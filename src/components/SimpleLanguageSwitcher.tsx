  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="group flex items-center gap-2 px-4 py-2 bg-slate-900/30 border border-slate-800/50 rounded-xl hover:bg-slate-900/50 hover:border-slate-700/50 transition-all duration-300 text-slate-300 hover:text-white"
      >
        ...
        <div className="absolute top-full ltr:right-0 rtl:left-0 mt-2 w-56 bg-slate-900/30 backdrop-blur-sm border border-slate-800/50 rounded-2xl shadow-xl overflow-hidden animate-in fade-in-0 zoom-in-95 duration-200">
          {languages.map((lang, index) => (
            <button
              ...
              className={`group w-full px-4 py-3 ltr:text-left rtl:text-right hover:bg-slate-800/50 transition-all duration-300 flex items-center gap-3 ${
                locale === lang.code 
                  ? 'bg-gradient-to-r from-slate-800/50 to-slate-700/50 text-white' 
                  : 'text-slate-300 hover:text-white'
              } ${lang.code === 'ar' ? 'flex-row-reverse' : ''} ${index === 0 ? 'rounded-t-2xl' : ''} ${index === languages.length - 1 ? 'rounded-b-2xl' : ''}`}
